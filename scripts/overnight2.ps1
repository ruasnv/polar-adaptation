...
# -- Methods (Optimized Selection for Code Domain) ----------------------------

$METHODS = @(
    "frozen",           # Baseline for domain shift
    "bitfit",           # Isolates residual-only impact
    "pure_paft",        # Efficiency ablation
    "safe_pure_paft",   # Efficiency hero for large shifts
    "lora_r8",          # Primary industry competitor
    "hybrid_paft",      # Native full-rank operator
    "safe_hybrid_paft"  # Full expressivity for large shifts
)

if ($IncludeFullFT) {
    $METHODS = $METHODS + @("full_finetune") # Performance ceiling
